...
old_block = open('js_block.txt', 'r', encoding='utf-8').read()

new_block = """async function runModel() {
    const inp = getInputs();
...
text = re.sub(r'const MODEL = \{.*?\};\n', '', text, flags=re.DOTALL)
text = re.sub(r'function predictNaturalDeorbit.*?\}\n\n', '', text, flags=re.DOTALL)
text = re.sub(r'function computeRiskScore.*?\}\n\n', '', text, flags=re.DOTALL)
text = re.sub(r'function getSailRecommendation.*?\}\n\n', '', text, flags=re.DOTALL)
text = re.sub(r'function eccWhy.*?\n\}\n', '', text, flags=re.DOTALL)
text = re.sub(r'function altWhy.*?\}\n', '', text, flags=re.DOTALL)
text = re.sub(r'function sizeWhy.*?\}\n', '', text, flags=re.DOTALL)
text = re.sub(r'function typeWhy.*?\}\n', '', text, flags=re.DOTALL)
